...
def audio_preprocessor(preview, base_audio, audio_wav, use_cuda=False):
    ...
    if preview:
        logger.warning(
            "Creating a preview video of 60 seconds, to disable "
            "this option, go to advanced settings and turn off preview."
        )
        wav_ = f'ffmpeg -y -i "{base_audio}" -ss 00:00:20 -t 00:01:00 -vn -acodec pcm_s16le -ar 44100 -ac 2 audio.wav'
    else:
...
def audio_video_preprocessor(
    ...
    if os.path.exists(video):
        if preview:
            ...
            mp4_ = f'ffmpeg -y -i "{video}" -ss 00:00:20 -t 00:01:00 -c:v libx264 -c:a aac -strict experimental Video.mp4'
        else:
        ...
        if preview:
            ...
            mp4_ = f'yt-dlp -f "mp4" --downloader ffmpeg --downloader-args "ffmpeg_i: -ss 00:00:20 -t 00:01:00" --force-overwrites --max-downloads 1 --no-warnings --no-playlist --no-abort-on-error --ignore-no-formats-error --restrict-filenames -o {OutputFile} {video}'
...
def old_audio_video_preprocessor(preview, video, OutputFile, audio_wav):
    ...
    if os.path.exists(video):
        if preview:
            ...
            command = f'ffmpeg -y -i "{video}" -ss 00:00:20 -t 00:01:00 -c:v libx264 -c:a aac -strict experimental Video.mp4'
        ...
        if preview:
            ...
            # https://github.com/yt-dlp/yt-dlp/issues/2220
            mp4_ = f'yt-dlp -f "mp4" --downloader ffmpeg --downloader-args "ffmpeg_i: -ss 00:00:20 -t 00:01:00" --force-overwrites --max-downloads 1 --no-warnings --no-abort-on-error --ignore-no-formats-error --restrict-filenames -o {OutputFile} {video}'
            wav_ = "ffmpeg -y -i Video.mp4 -vn -acodec pcm_s16le -ar 44100 -ac 2 audio.wav"
            result_convert_video = subprocess.run(
